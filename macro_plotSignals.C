...
int macro_plotSignals ()
{
  ...
  expo_3->SetParameter (0, 3.69582) ;
  expo_3->SetParameter (1, -4.7226) ;
  expo_3->SetParameter (2, -0.0427676) ; 
  expo_3->SetParameter (3, 4.20259e-05) ;
  expo_3->SetParameter (4, -1.6383e-08) ;
  ...
  for (double mass = 300 ; mass < 1100 ; mass += 25)
    {
      TF1 * func = new TF1 ("func",crystalBallLowHigh, 200, 2000, 7) ;
      double params[7] = {expo_3->Eval (mass), tg_sig_par1->Eval (mass), tg_sig_par2->Eval (mass), 
                          tg_sig_par3->Eval (mass), tg_sig_par4->Eval (mass), tg_sig_par5->Eval (mass), 
                          tg_sig_par6->Eval (mass)} ;
      func->SetParameters (params) ;
      func->SetLineWidth (1) ;
