  ...
  histos (TString name, double XS) : m_name (name), m_XS (XS)
    {
      ...
      m_h_scale->Sumw2 () ;
      mh_MWW = new TTree (TString ("hMWW_") + name, 
                          TString ("hMWW_") + name) ;
    }
...
fillHistos (LHEF::Reader & reader, histos & Histos, double XS, double referenceScale = 0, int max = -1)
{
  ...
  float mWW = 0.;
  Histos.mh_MWW->Branch("mWW",&mWW,"mWW/F"); 
   ...
   if (reader.hepeup.ISTUP.at (iPart) == 2) {
    ...
    if (abs (reader.hepeup.IDUP.at (iPart)) == 6) {
     ...
     continue;
      ...
      if (v_f_quarks.at (detaIndices.second).Eta () - v_f_quarks.at (detaIndices.first).Eta () < 2) continue ; //selection applied in madgraph
...
//      pair<int, int> Wpair (2, 3) ;
      ...
      lorentzVector VBFPair;

      if (Wpair.first == 0)
	{  if (Wpair.second ==1)
	        VBFPair = v_f_quarks.at (2) + v_f_quarks.at (3) ; 
	  else if (Wpair.second ==2)
	        VBFPair = v_f_quarks.at (1) + v_f_quarks.at (3) ; 	    
	  else if (Wpair.second ==3)
	        VBFPair = v_f_quarks.at (1) + v_f_quarks.at (2) ; 	    
	}

      else if (Wpair.first == 1)
	{  if (Wpair.second ==2)
	        VBFPair = v_f_quarks.at (0) + v_f_quarks.at (3) ; 
	  else if (Wpair.second ==3)
	        VBFPair = v_f_quarks.at (0) + v_f_quarks.at (2) ; 	    
	}

      else if (Wpair.first == 2)
	{  if (Wpair.second ==3)
	        VBFPair = v_f_quarks.at (0) + v_f_quarks.at (1) ; 
	}

      if ( VBFPair.M() < 200) continue;
...
//  while (!inputstream.eof ()) 
  ...
  string cprime = argv[3] ;
  if (cprime != "0p1" && cprime != "0p3" && cprime != "0p5" && cprime != "0p7" && cprime != "0p9" && cprime != "SM")
    {
      cout << "wrong cprime: " << cprime << endl ;
      exit (1) ;
    }
